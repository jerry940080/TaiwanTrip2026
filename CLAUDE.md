...
`simple.html` 是給長輩看的大字版，`台灣行程.docx` 是可以列印的紙本版（見下面「三種版本」）。
...
- **`台灣行程.docx`（紙本版）**——**把 `index.html` 的畫面切成一頁一張貼進 Word**，
  不是重新排版的文字，所以照片、地圖、配色都跟螢幕上一樣。改完行程要重跑：
  ...
  `assets/print/` 是產生物，沒進版控。細節（為什麼渲染寬度是 680px、切頁怎麼避開
  標題與照片、圖片格式怎麼挑）看 `TEMPLATE_GUIDE.md`。
...
- **`台灣行程.docx`（紙本版）**——從 `index.html` **自動產生**，內容跟完整版一樣，
  只是排成 A4、字放大、每天從新的一頁開始，最後附三頁空白筆記。改完行程要重跑：

  ```bash
  NODE_PATH=<playwright 位置> node tools/shoot_maps.js   # 地圖截圖，只有地圖改了才要跑
  python3 tools/make_docx.py                             # 產生 docx
  ```

  `tools/make_docx.py` 只用標準庫直接寫 OOXML，不必裝 python-docx；
  資料靠 `tools/jsdata.py` 把 `index.html` 的 JS 物件實字讀成 Python 資料。
