...
不是固定高度切下去，不然標題、照片、表格的一列都會被剖成兩半，而且一頁會同時出現
「第 X 天的尾巴」與「第 X+1 天的開頭」——翻起來完全搞不清楚在看哪一天。
`shoot_pages.js` 分兩層處理：

**第一層：單元。** 封面、行前、每一天各是一個單元，**單元邊界一定是頁的邊界**。
所以任何一頁都只會有一天的東西，翻頁不會翻到一半換成別天。
最後的參考區塊（每天吃什麼、訂票、出發前確認、三段巴士、頁尾）不是「一天」，
讓它們照舊連續往下排——各自獨立成單元只會多出好幾頁半空白的紙。

**第二層：單元內部分頁。**

1. **收集不可以切開的東西**（`.day-head`、`.mapcard`、`.ph`、`tr`、`li`、`p`、`h1`–`h3`…），
   任何落在它們內部的切點直接淘汰。
2. 一個單元要 `n = ceil(高度 / HMAX)` 頁，**平均分成 n 份**，每個切點取離等分位置最近的
   合法切點。平均分而不是把前面塞滿，兩頁的那一天才不會第一頁滿、第二頁只有兩行。
3. `HMAX = 頁高 × 1.08`。超出的部分由 `make_docx.py` 等比縮小吸收（它本來就會把
   `cy > IMG_H_MAX` 的圖縮進頁面）。縮 8% 以內看不出來，但可以讓「差一點就一頁」
   的日子真的變成一頁。

目前 35 頁：六天剛好兩頁，9/27 與 9/29 三頁，9/30 與 10/1 四頁——那兩天有兩張地圖
（`map` 與 `map2`，光地圖區就 1,084px）加上六張景點卡，內容就是比較多。
**想壓頁數只能拿字級換**：`PRINT_W` 環境變數可以改渲染寬度，760 約 10.7pt、820 約 10pt
（820 時 9/27 進兩頁、9/30 進三頁，總共 30 頁）。預設 680 不動，因為紙本是給長輩看的。

### 圖片格式是逐頁挑的

每頁 PNG 與 JPEG 各存一次、留比較小的那個。整頁都是字的頁面顏色少，**PNG 反而比 JPEG 小**；
有照片的頁面則是 JPEG 小很多。兩種印出來都看不出差別。現在 35 頁裡 PNG 與 JPEG 各半，
總共約 11 MB。
