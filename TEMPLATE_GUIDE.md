...
另外 `cfg.padLeft`（0–1）會在放大時於左側預留一段空白——首頁大圖左邊壓著標題文字，
設 `.40` 可以讓放大後的地形落在右側、不被字蓋住。日圖沒有文字遮擋，不用設。

範例（首頁總覽圖在南下時鏡頭跟著移到南投，北返時拉回全圖）：

```js
{a:'home',b:'taichungHsr',mode:'hsr',zoom:{la0:23.50,la1:24.30,lo0:120.40,lo1:121.15}},
{a:'xitou',b:'taichungHsr',mode:'bus',pause:1600},
{a:'taichungHsr',b:'home',mode:'hsr',zoom:'reset'},
```

尊重系統的「減少動態」設定：開啟時直接顯示完整路線，不做平移。
